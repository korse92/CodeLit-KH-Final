...
public class HelloSpringUtils {
	 ...
	 * pageBarSize 페이지바의 페이지 수 pageStart ~ pageEnd 페이지 바 범위 pageNo 증감변수
	 * 
	 * < 1 2 3 4 5 > : 이전버튼 비활성화 < 6 7 8 9 10 > < 11 12 > : 다음버튼 비활성화
	 * 
	 */
	public static String getPageBar(int totalContents, int cPage, int numPerPage, String url) {
		...
		// 사용자입력값 있는경우는 & 아니면 ?
		/**
		 * 현재 url은 String형 -> String 클래스의 indexIF 메소드 사용. indexOf() 메서드는 호출한 String
		 * 객체(문자열)에서 주어진 값과 일치하는 첫 번째 인덱스를 반환. 일치하는 값이 없으면 -1을 반환 indexOf로 검사시 ? 의 위치가
		 * 나오는데 위치는 무조건 0보다 같거나 크다 맨왼쪽에 있으면 0이고 1씩 늘어난다 -> 없으면 -1을 리턴한다
		 */

		// url이 String형인데,
		// String 클래스에 indexOf 메소드가 있거든요. 그래서 모든 문자열에 대고 indexOf 메소드를 쓸 수 있는거고,,
		// 그래서 url.indexOf("?") 일케 쓸 수 있는데, 여기 매개인자로 넣어준게 uri에 있는지 찾아주는거고, 있으면 어디 위치에
		// 있는지 그 위치(index)를 리턴해서 줘요.
